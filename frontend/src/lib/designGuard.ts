...
// 与服务端 COMPONENT_WORDS / EFFECT_WORDS 同源（双向契约测试保证同步）
const COMPONENT_WORDS = ['卡片', '按钮', '标题', '图片', '表格', '导航', '侧边栏', '输入框', '标签']
const EFFECT_WORDS = ['阴影', '投影', '渐变', '动效', '动画', '圆角', '描边', '边框', '背景', '变换']
...
export function isDesignRequest(prompt: string): boolean {
  ...
  const hasComponent = COMPONENT_WORDS.some((k) => prompt.includes(k))
  const hasEffect = EFFECT_WORDS.some((k) => prompt.includes(k))
